...
NRPE bietet eine möglichkeit, Monitoring Plugins auf einem Remote System auszuführen. Dies setzt einen laufenden NRPE-Server und Installierte Monitoring Plugins auf dem Remote System vorraus.
...
Stellen Sie eine SSH verbindung mit ihrem openITCOCKPIT Server her und installieren die das folgende Paket: 
```
apt-get install --no-install-recommends nagios-nrpe-plugin
```

Es wird empfohlen das `check_nrpe` Plugin in den Standard Plugin Ordner von openITCOCKPIT zu Linken
...
| Service name | `CHECK_BY_NRPE_SIMPLE` |
| Check period | `24x7` |
...
Die Standardkonfiguration kommt mit vordefinierten basis Checks. Wir nutzen diese Beispiel Checks nun.

In der Standardkonfiguration wird der Check die Festplatte `/dev/hda1` überprüfen, welche es auf den meissten systemen nicht mehr gibt. In unserem fall ersetzen wir dies durch `/dev/vda1`.
...
Möchten Sie die Absender adresse der E-Mails ändern, Navigieren Sie nach `Systemkonfiguration -> System -> Systemeinstellungen`.
